...
def test_canonical_json_bytes_stable_regardless_of_key_order() -> None:
...
def test_canonical_json_bytes_supported_types() -> None:
...
def test_canonical_json_bytes_dict_with_datetime_fails_hard() -> None:
    """Dict with non-JSON-serializable (e.g. datetime) fails hard."""
    with pytest.raises(
        TypeError,
        match=r"Object of type datetime is not JSON serializable|not serializable",
    ):
...
def test_canonical_json_bytes_fails_on_nan(value: float, match: str) -> None:
    """Raises on NaN/Infinity (allow_nan=False)."""
    with pytest.raises(ValueError, match=match):
        canonical_json_bytes(value)


def test_canonical_json_bytes_base_model() -> None:
...
def test_hash_payload_bytes() -> None:
