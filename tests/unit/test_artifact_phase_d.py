"""Phase D: put_file + open_path — file artifact, hash, path traversal blocked."""
...
from lily.kernel import ArtifactStore, PutArtifactOptions, create_run


def test_file_artifact_accessible(workspace_root: Path) -> None:
    ...
    ref = store.put_file(
        "blob.v1", source, options=PutArtifactOptions(artifact_name="input")
    )
...
def test_put_file_copy_default(workspace_root: Path) -> None:
...
def test_put_file_move_true(workspace_root: Path) -> None:
...
def test_file_hash_correct(workspace_root: Path) -> None:
...
def test_open_path_traversal_blocked(workspace_root: Path) -> None:
...
def test_open_path_rejects_json_artifact(workspace_root: Path) -> None:
    """open_path raises for non-file storage_kind."""
    run_id, run_root = create_run(workspace_root)
    store = ArtifactStore(run_root, run_id)
    ref = store.put_json("work_order.v1", {"x": 1})
    assert ref.storage_kind.value == "json"
    with pytest.raises(ValueError, match="requires storage_kind=file"):
        store.open_path(ref)
