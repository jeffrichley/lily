...
def _format_help(entry: SkillEntry) -> str:
    ...
    lines = [
        ...
        "- `plugin`:",
        f"  - `entrypoint`: {entry.plugin.entrypoint or '(none)'}",
        f"  - `source_files`: {_format_list(entry.plugin.source_files)}",
        f"  - `asset_files`: {_format_list(entry.plugin.asset_files)}",
        f"  - `profile`: {entry.plugin.profile}",
        f"  - `write_access`: {str(entry.plugin.write_access).lower()}",
        f"  - `env_allowlist`: {_format_list(entry.plugin.env_allowlist)}",
        "- `eligibility`:",
